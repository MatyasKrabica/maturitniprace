...
if ($action === 'open' && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['answer_text']) && $ticketId > 0) {
    ...
    if ($ticketInfo) {
        ...
        if ($isClosed && !$isAdminOrSupport) {
            $canAnswer = false;
            $errors[] = "Nemůžete odpovídat na uzavřený ticket.";
        }
        ...
        if ($canAnswer && !$isEmpty) {
            ...
            if ($success) {
                ...
                $errors[] = "Nepodařilo se odeslat odpověď.";
            }
        } elseif ($isEmpty) {
            $errors[] = "Text odpovědi nesmí být prázdný.";
        }
    } else {
        $errors[] = "Ticket neexistuje.";
    }
}
...
if ($action === 'create' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($ticketName === '' || $isEmpty || $categoryId <= 0) {
        $errors[] = "Vyplňte název, text ticketu a vyberte kategorii.";
    } else {
        ...
        if ($newId) {
            ...
            $errors[] = "Chyba při vytváření ticketu do databáze.";
        }
    }
}
?>
...
<head>
<meta charset="utf-8">
<title>Maturitní práce | Podpora</title>
...
<div>
<h1 class="text-3xl font-black text-gray-900 dark:text-white italic tracking-tight mb-2">Moje Tickety</h1>
<p class="text-gray-500 text-sm">Přehled vaší komunikace s podporou.</p>
</div>
...
<h3 class="text-xl font-bold text-gray-900 dark:text-white mb-2">Žádné tickety</h3>
<p class="text-gray-500">Zatím jste nevytvořili žádný požadavek na podporu.</p>
</div>
<?php else: ?>
...
<th class="py-4 px-4 font-bold w-1/3">Předmět</th>
<th class="py-4 px-4 font-bold">Autor</th>
<th class="py-4 px-4 font-bold">Kategorie</th>
<th class="py-4 px-4 font-bold">Status</th>
<th class="py-4 px-4 font-bold text-right">Akce</th>
</tr>
</thead>
...
<td class="py-4 px-4 text-gray-500"><?= htmlspecialchars($t['category_name'] ?? '-') ?></td>
<td class="py-4 px-4">
<?php
$statusColors = [
...
<?= htmlspecialchars($t['status_text']) ?>
</span>
</td>
<td class="py-4 px-4 text-right">
<a href="ticket.php?action=open&id=<?= $t['id'] ?>" class="text-gray-400 hover:text-gray-900 dark:hover:text-white font-bold text-xs uppercase tracking-wider">Otevřít &rarr;</a>
</td>
</tr>
<?php endforeach; ?>
</tbody>
...
<h2 class="text-3xl font-black text-gray-900 dark:text-white italic tracking-tight mb-8">Vytvořit Ticket</h2>

<form method="post" onsubmit="return submitQuillForm('ticketQuillEditor', 'ticket_text')" class="space-y-6">
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
<div class="space-y-2">
<label class="text-[10px] uppercase font-bold text-gray-500 tracking-widest">Kategorie</label>
...
<label class="text-[10px] uppercase font-bold text-gray-500 tracking-widest">Specifikace</label>
<input type="text" id="customReasonInput" name="custom_reason" placeholder="Uveďte důvod..." class="w-full bg-gray-50 dark:bg-[#121214] border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-yellow-500 transition-colors text-gray-900 dark:text-white">
</div>
</div>

<div class="space-y-2" id="ticketNameLabel">
<label class="text-[10px] uppercase font-bold text-gray-500 tracking-widest">Předmět</label>
<input type="text" id="ticketNameInput" name="ticket_name" required class="w-full bg-gray-50 dark:bg-[#121214] border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-yellow-500 transition-colors text-gray-900 dark:text-white font-bold" placeholder="Stručný popis problému">
</div>

<div class="space-y-2">
<label class="text-[10px] uppercase font-bold text-gray-500 tracking-widest">Zpráva</label>
...
if (!$info): ?>
    <div class="glass p-12 text-center rounded-3xl">Ticket nenalezen.</div>
    <?php else:
    $isAuthor = ($currentUsername === $info['username']);

if (!$isAuthor && !$isAdminOrSupport): ?>
    <div class="glass p-12 text-center rounded-3xl text-red-500 font-bold">Nemáte oprávnění.</div>
    <?php else:
    $answers = $ticketManager->getTicketAnswers($ticketId);
$isClosed = ((int)$info['status'] === TicketManager::STATUS_CLOSED);
$catName = htmlspecialchars($info['category_name'] ?? 'Neznámá');
if ((int)$info['category_id'] === $otherCategoryId) $catName = "Jiný důvod";
?>
...
Stav: <?= htmlspecialchars($info['status_text']) ?>
</span>
</div>
</div>
...
<span class="text-[10px] font-bold uppercase text-gray-400 bg-gray-100 dark:bg-white/5 px-2 py-0.5 rounded">Autor</span>
...
<?php if($isAdminAns): ?>
<span class="text-[10px] font-bold uppercase text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded">Support</span>
<?php endif; ?>
...
<span class="text-gray-400 text-xs <?= $isAdminAns ? 'mr-auto' : 'ml-auto' ?> flex items-center gap-2">
    ...
    <?php if ($isAdminOrSupport): ?>
        <a href="ticket.php?action=open&id=<?= $ticketId ?>&delete_ans_id=<?= $ans['id'] ?>" 
           onclick="return confirm('Opravdu smazat tuto odpověď?')"
...
<h3 class="text-sm font-bold uppercase text-gray-400 tracking-widest mb-4">Odpovědět</h3>
...
<h3 class="font-black text-gray-900 dark:text-white uppercase text-xs tracking-widest mb-4">Administrace</h3>
<div class="flex flex-col gap-2">
<?php if (!$isClosed): ?>
<a href="ticket.php?action=close&id=<?= $info['id'] ?>" onclick="return confirm('Opravdu uzavřít?')" class="w-full bg-yellow-500 hover:bg-yellow-600 text-black py-3 rounded-xl font-bold text-xs uppercase tracking-wider text-center transition">
Uzavřít Ticket
</a>
<?php else: ?>
<a href="ticket.php?action=reopen&id=<?= $info['id'] ?>" onclick="return confirm('Opravdu znovu otevřít?')" class="w-full bg-emerald-500 hover:bg-emerald-600 text-black py-3 rounded-xl font-bold text-xs uppercase tracking-wider text-center transition">
Znovu otevřít
</a>
<?php endif; ?>
<a href="ticket.php?action=delete&id=<?= $info['id'] ?>" onclick="return confirm('Opravdu SMAZAT ticket?')" class="w-full bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white py-3 rounded-xl font-bold text-xs uppercase tracking-wider text-center transition">
Smazat Ticket
</a>
</div>
</div>
...
<h3 class="font-black text-gray-900 dark:text-white uppercase text-xs tracking-widest mb-4">Informace</h3>
...
<p class="text-[10px] uppercase font-bold text-gray-400 dark:text-gray-700 tracking-[0.4em]">&copy; 2026 Maturitní práce • Všechna práva vyhrazena</p>
</footer>
