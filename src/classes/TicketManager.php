...
class TicketManager {
    ...
    public const STATUSES = [
        ...
        self::STATUS_AWAITING_USER => 'Čeká se na uživatele',
        self::STATUS_AWAITING_SUPPORT => 'Čeká se na support',
    ];
    ...
    public function getAllTickets(): array {
        ...
        if ($res) {
            while ($r = mysqli_fetch_assoc($res)) {
                $r['status_text'] = self::STATUSES[$r['status']] ?? 'Neznámý';
                $tickets[] = $r;
            }
        }
        return $tickets;
    ...
    public function getTicketById(int $ticketId): ?array {
        ...
        if ($ticket) {
            $ticket['status_text'] = self::STATUSES[$ticket['status']] ?? 'Neznámý';
        }
