...
<body class="antialiased min-h-screen transition-colors duration-300">
        ...
        <aside class="w-full md:w-64 md:fixed md:h-screen glass border-r border-gray-200 dark:border-white/5 p-6 flex flex-col gap-8 z-50 shadow-sm dark:shadow-none transition-colors duration-300">
            ...
            <div class="mt-auto flex flex-col gap-3">
                <button onclick="toggleTheme()" class="flex items-center justify-center p-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 hover:bg-yellow-500/20 transition-all group font-bold text-gray-600 dark:text-gray-300 text-sm">
                    <span class="dark:hidden group-hover:text-yellow-500 transition-colors"><i class="fa-solid fa-moon mr-2"></i><?= t('admin_theme_dark', 'Tmavý režim') ?></span>
                    <span class="hidden dark:block text-yellow-500"><i class="fa-solid fa-sun mr-2"></i><?= t('admin_theme_light', 'Světlý režim') ?></span>
                </button>
                <a href="dashboard.php" class="text-red-500 p-4 block text-center font-bold hover:bg-red-500/10 rounded-xl transition-colors"><i class="fa-solid fa-arrow-left mr-2"></i><?= t('admin_back_app', 'Zpět do appky') ?></a>
            </div>
        </aside>
    ...
    <div>
        <div class="flex items-center justify-between mb-6">
            <h3 class="text-gray-500 font-bold uppercase text-xs tracking-widest"><?= t('admin_sync_api_label', 'Google Fit API') ?></h3>
        ...
        <h2 class="text-3xl font-black text-gray-900 dark:text-white uppercase italic tracking-tighter mb-4">
            <?= t('admin_sync_title_1', 'Manuální') ?><br><span class="text-yellow-500"><?= t('admin_sync_title_2', 'Synchronizace') ?></span>
        </h2>
        
        <p class="text-gray-600 dark:text-gray-400 text-sm font-light leading-relaxed mb-6">
            <?= t('admin_sync_desc', 'Tato akce okamžitě spojí systém se servery Google Fit a vynutí stažení nejnovějších dat o krocích pro všechny aktivní uživatele.') ?>
        </p>
        ...
        <div class="bg-blue-500/5 dark:bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 mb-8">
            <p class="text-[10px] text-blue-600 dark:text-blue-400 uppercase font-bold tracking-widest flex items-center gap-2">
                <i class="fa-solid fa-circle-info"></i> <?= t('admin_sync_info_title', 'Info k systému') ?>
            </p>
            <p class="text-xs text-gray-700 dark:text-gray-300 mt-2 font-medium">
                <?= t('admin_sync_info_text', 'Běžná synchronizace je nastavena automaticky přes CRON. Toto tlačítko používejte převážně v případě výpadku nebo zpoždění dat.') ?>
            </p>
        </div>
    </div>
    ...
    <form method="POST" class="mt-auto">
        <button type="submit" name="run_manual_sync" class="w-full bg-gray-900 dark:bg-white text-white dark:text-black py-5 rounded-2xl font-black uppercase hover:bg-yellow-500 dark:hover:bg-yellow-500 transition-all shadow-xl shadow-gray-900/10 dark:shadow-white/10 text-sm tracking-widest group">
            ...
            <?= t('admin_sync_btn', 'Spustit manuální sync') ?>
        </button>
    </form>
</div>
                    ...
                    <div class="glass p-8 rounded-[2rem] lg:col-span-2 shadow-sm dark:shadow-none">
                        <div class="flex justify-between items-center mb-6">
                            <h3 class="text-gray-500 font-bold uppercase text-xs tracking-widest"><?= t('admin_logs_title', 'Systémové Logy') ?></h3>
                            <span class="text-[10px] bg-white dark:bg-white/5 border border-gray-200 dark:border-white/5 px-3 py-1 rounded-full text-gray-500 dark:text-gray-400 font-bold"><?= t('admin_logs_last', 'Posledních 50 akcí') ?></span>
                        </div>
                        ...
                        <div class="max-h-[400px] overflow-y-auto custom-scroll pr-2">
                            <table class="w-full text-left text-sm">
                                <thead class="sticky top-0 bg-gray-100 dark:bg-[#0a0a0c] text-[10px] uppercase text-gray-500 font-bold z-10 transition-colors duration-300">
                                    <tr><th class="pb-4 pt-2"><?= t('admin_logs_user_col', 'Uživatel') ?></th><th class="pb-4 pt-2"><?= t('admin_logs_action_col', 'Akce') ?></th><th class="pb-4 pt-2 text-right"><?= t('admin_logs_time_col', 'Čas') ?></th></tr>
                                </thead>
            ...
            <?php elseif ($section === 'users'): ?>
                <h1 class="text-4xl font-black text-gray-900 dark:text-white uppercase italic mb-8 tracking-tighter"><?= t('admin_users_title', 'Uživatelé') ?></h1>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            <thead class="bg-white dark:bg-white/5 text-[10px] uppercase text-gray-500 font-bold border-b border-gray-200 dark:border-white/5">
                                <tr><th class="p-6"><?= t('admin_user_col', 'Uživatel') ?></th><th class="p-6"><?= t('admin_rank_col', 'Rank') ?></th><th class="p-6 text-right"><?= t('admin_actions_col', 'Akce') ?></th></tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                                ...
                                <tr class="hover:bg-white dark:hover:bg-white/5 transition-colors">
                                    ...
                                    <td class="p-6 text-right"><a href="admin.php?section=edit_user&id=<?=$u['id']?>" class="bg-yellow-500 hover:bg-yellow-600 text-black px-4 py-2 rounded-lg text-xs font-bold uppercase transition-colors shadow-md"><?= t('admin_edit_btn', 'Upravit') ?></a></td>
                                </tr>
                                <?php endwhile; ?>
                            </tbody>
                        </table>
            ...
            <?php elseif ($section === 'edit_user'): ?>
                <?php
                    ...
                    if (!$eu): ?>
                        <p class="text-red-500 font-bold"><?= t('admin_user_not_found', 'Uživatel nenalezen.') ?></p>
                    <?php else: ?>
                <div class="flex items-center justify-between mb-8">
                    <div>
                        <a href="admin.php?section=users" class="text-yellow-500 hover:text-yellow-600 text-sm font-bold uppercase tracking-widest transition-colors"><i class="fa-solid fa-arrow-left mr-2"></i><?= t('admin_back_list', '← Zpět na seznam') ?></a>
                        <h1 class="text-4xl font-black text-gray-900 dark:text-white uppercase italic tracking-tighter mt-2">
                            <?= t('admin_edit_user_title', 'Úprava:') ?> <span class="text-yellow-500"><?= htmlspecialchars($eu['username']) ?></span>
                            <span class="text-gray-400 dark:text-gray-600 font-normal text-2xl ml-1">#<?= $eu['id'] ?></span>
                        </h1>
                    </div>
                    <?php if (isset($_GET['saved'])): ?>
                        <span class="bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 px-4 py-2 rounded-xl text-sm font-bold"><i class="fa-solid fa-check mr-2"></i><?= t('admin_saved_badge', 'Uloženo') ?></span>
                    <?php endif; ?>
                </div>
                ...
                <form method="POST" class="space-y-8">
                    ...
                    <div class="glass p-8 rounded-[2rem] shadow-sm dark:shadow-none border border-gray-200 dark:border-white/5">
                        <h3 class="text-xs font-bold uppercase tracking-widest text-yellow-500 mb-6"><i class="fa-solid fa-user mr-2"></i><?= t('admin_personal_section', 'Osobní údaje') ?></h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-gray-500 mb-2 tracking-widest"><?= t('admin_username_label', 'Uživatelské jméno') ?></label>
                                ...
                                <p class="text-[10px] text-gray-400 mt-1 ml-1"><?= t('admin_username_hint', 'Jméno nelze měnit z admin panelu') ?></p>
                            </div>
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-gray-500 mb-2 tracking-widest"><?= t('admin_email_label', 'Email') ?></label>
                            ...
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-gray-500 mb-2 tracking-widest"><?= t('admin_firstname_label', 'Jméno') ?></label>
                                <input type="text" name="first_name" value="<?= htmlspecialchars($eu['first_name']) ?>"
                                       class="w-full p-4 rounded-xl font-medium bg-white dark:bg-[#121214] border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white outline-none focus:border-yellow-500 transition-colors">
                            </div>
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-gray-500 mb-2 tracking-widest"><?= t('admin_lastname_label', 'Příjmení') ?></label>
                    ...
                    <div class="glass p-8 rounded-[2rem] shadow-sm dark:shadow-none border border-gray-200 dark:border-white/5">
                        <h3 class="text-xs font-bold uppercase tracking-widest text-yellow-500 mb-6"><i class="fa-solid fa-map-location-dot mr-2"></i><?= t('admin_location_section', 'Působiště') ?></h3>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-gray-500 mb-2 tracking-widest"><?= t('register_country','Země') ?></label>
                            ...
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-gray-500 mb-2 tracking-widest"><?= t('register_region','Kraj') ?></label>
                            ...
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-gray-500 mb-2 tracking-widest"><?= t('register_city','Město') ?></label>
                        ...
                        <p class="text-xs text-gray-400 mt-3 ml-1"><?= t('admin_currently_label', 'Aktuálně') ?>: <?= htmlspecialchars(($eu['city_name'] ?? '–') . ', ' . ($eu['region_name'] ?? '–')) ?></p>
                        <?php endif; ?>
                    </div>

                    <div class="glass p-8 rounded-[2rem] shadow-sm dark:shadow-none border border-gray-200 dark:border-white/5">
                        <h3 class="text-xs font-bold uppercase tracking-widest text-yellow-500 mb-6"><i class="fa-solid fa-shield-halved mr-2"></i><?= t('admin_system_section', 'Systémová nastavení') ?></h3>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-gray-500 mb-2 tracking-widest"><?= t('admin_rank_perm_label', 'Rank (oprávnění)') ?></label>
                            ...
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-emerald-600 dark:text-emerald-500 mb-2 tracking-widest"><?= t('admin_level_label', 'Level (role)') ?></label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 pt-6 border-t border-gray-100 dark:border-white/5">
                            <div class="flex items-center gap-4">
                                ...
                                <span class="text-sm font-bold text-gray-700 dark:text-gray-300"><?= t('admin_banned_label', 'Zablokován (ban)') ?></span>
                            </div>
                            <div>
                                <label class="block text-[10px] uppercase font-bold text-red-600 dark:text-red-500 mb-2 tracking-widest"><?= t('admin_banned_until_label', 'Ban do (datum)') ?></label>
                    ...
                    <div class="glass p-6 rounded-[2rem] shadow-sm dark:shadow-none border border-gray-200 dark:border-white/5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-gray-400">
                            <div><strong><?= t('admin_registration_label', 'Registrace') ?>:</strong> <?= date('d.m.Y H:i', strtotime($eu['created_at'])) ?></div>
                            <div><strong>Google Fit:</strong> <?= $eu['fit_status'] ? '<span class="text-emerald-500">' . t('admin_google_fit_active','Aktivní') . '</span>' : '<span class="text-gray-500">' . t('admin_google_fit_inactive','Neaktivní') . '</span>' ?></div>
                            <div><strong><?= t('admin_profile_image_label', 'Profilový obrázek') ?>:</strong> <?= htmlspecialchars($eu['profile_image'] ?? '1.png') ?></div>
                    ...
                    <button type="submit" name="update_user_full"
                        ...
                        <i class="fa-solid fa-save mr-2"></i><?= t('admin_save_changes_btn', 'Uložit změny') ?>
                    </button>
                </form>
                ...
                (function(){
                    ...
                    async function euLoadCountries() {
                        ...
                        euFill(cs, data, 'code', 'name', '<?= addslashes(t('register_select_country','Vyber zemi')) ?>');
                    ...
                    async function euLoadRegions(cc) {
                        ...
                        rs.innerHTML = '<option value=""><?= addslashes(t('register_select_region','-- Vyber kraj --')) ?></option>';
                        ms.innerHTML = '<option value=""><?= addslashes(t('register_select_city','-- Vyber město --')) ?></option>';
                        if (!cc) return;
                        const data = await euFetch('src/ajax/ajax_regions.php?country_code=' + encodeURIComponent(cc));
                        euFill(rs, data, 'id', 'name', '<?= addslashes(t('register_select_region','Vyber kraj')) ?>');
                    ...
                    async function euLoadCities(rid, cc) {
                        ...
                        euFill(ms, data, 'id', 'name', '<?= addslashes(t('register_select_city','Vyber město')) ?>');
            ...
            <?php elseif ($section === 'challenges'): ?>
                ...
                ?>
                <h1 class="text-4xl font-black text-gray-900 dark:text-white uppercase italic mb-8 tracking-tighter"><?= $editMode ? t('admin_challenge_edit_title','Editace Výzvy') : t('admin_challenge_new_title','Nová Výzva') ?></h1>
                ...
                <div class="glass p-8 rounded-[2rem] mb-12 shadow-sm dark:shadow-none border border-gray-200 dark:border-white/5">
                    <form method="POST" class="space-y-6">
                        ...
                        <!-- Název -->
                        <div>
                            <label class="text-[10px] uppercase font-bold text-gray-500 ml-2 tracking-widest"><?= t('admin_challenge_name_ph','Název výzvy') ?></label>
                            <input type="text" name="title" required value="<?= $editMode ? htmlspecialchars($cData['title']) : '' ?>" placeholder="<?= htmlspecialchars(t('admin_challenge_name_ph','Název výzvy'),ENT_QUOTES) ?>" class="w-full p-4 mt-1 rounded-xl font-bold bg-white dark:bg-[#121214] border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white outline-none focus:border-yellow-500 transition-colors">
                        </div>

                        <!-- Popis -->
                        <div>
                            <label class="text-[10px] uppercase font-bold text-gray-500 ml-2 tracking-widest"><?= t('admin_challenge_desc_label','Popis') ?></label>
                            <textarea name="description" rows="3" class="w-full p-4 mt-1 rounded-xl font-medium bg-white dark:bg-[#121214] border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white outline-none focus:border-yellow-500 transition-colors resize-none"><?= $editMode ? htmlspecialchars($cData['description'] ?? '') : '' ?></textarea>
                        </div>

                        <!-- Typ aktivity -->
                        <div>
                            <label class="text-[10px] uppercase font-bold text-blue-600 dark:text-blue-400 ml-2 tracking-widest"><?= t('challenge_activity_type','Typ aktivity') ?></label>
                            <p class="text-[10px] text-gray-400 ml-2 mb-1"><?= t('admin_activity_type_hint','Kroky jsou počítány POUZE z tohoto typu aktivity v Google Fit.') ?></p>
                            <select name="activity_type" class="w-full p-4 rounded-xl font-bold bg-white dark:bg-[#121214] border border-blue-500/30 text-gray-900 dark:text-white outline-none focus:border-blue-500 transition-colors">
                                <?php foreach (Settings::ACTIVITY_TYPES as $actId => $actKey): ?>
                                    <option value="<?= $actId ?>" <?= ($editMode && (int)$cData['activity_type'] === $actId) ? 'selected' : (!$editMode && $actId === 7 ? 'selected' : '') ?>>
                                        <?= t($actKey, $actKey) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div>
                                <label class="text-[10px] uppercase font-bold text-gray-500 ml-2 tracking-widest"><?= t('admin_challenge_steps_label','Cílové kroky') ?></label>
                            ...
                            <div>
                                <label class="text-[10px] uppercase font-bold text-gray-500 ml-2 tracking-widest"><?= t('admin_challenge_time_label','Čas. limit (hod)') ?></label>
                                <input type="number" name="time_limit_hours" min="1" value="<?= $editMode ? $cData['time_limit_hours'] : '72' ?>" class="w-full p-4 mt-1 rounded-xl font-bold bg-white dark:bg-[#121214] border border-gray-200 dark:border-white/10 text-gray-900 dark:text-white outline-none focus:border-yellow-500 transition-colors">
                            </div>
                        </div>

                        <!-- Lokalita -->
                        <div>
                            <label class="text-[10px] uppercase font-bold text-gray-500 ml-2 tracking-widest"><?= t('admin_location_section','Působiště') ?></label>
                        ...
                        <button type="submit" name="<?= $editMode?'update_challenge':'create_challenge' ?>" class="w-full bg-emerald-500 hover:bg-emerald-600 text-black font-black py-5 rounded-2xl uppercase italic shadow-lg shadow-emerald-500/20 transition-all text-sm tracking-widest">
                            <?= $editMode ? t('admin_challenge_save_btn','Uložit změny') : t('admin_challenge_create_btn','Vytvořit výzvu') ?>
                        </button>
                    </form>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            <thead class="bg-white dark:bg-white/5 text-[10px] uppercase font-bold text-gray-500 border-b border-gray-200 dark:border-white/5">
                                <tr><th class="p-6"><?= t('admin_challenge_col', 'Výzva') ?></th><th class="p-6"><?= t('admin_reward_penalty_col', 'Reward / Penalty') ?></th><th class="p-6 text-right"><?= t('admin_actions_col', 'Akce') ?></th></tr>
                            </thead>
                            <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                                ...
                                <tr class="hover:bg-white dark:hover:bg-white/5 transition-colors">
                                    ...
                                    <td class="p-6 text-right space-x-2">
                                        ...
                                        <a href="admin.php?section=challenges&delete_challenge=<?= $r['id'] ?>" class="inline-block p-3 bg-red-100 dark:bg-red-500/10 text-red-600 dark:text-red-500 rounded-xl hover:bg-red-200 dark:hover:bg-red-500/20 transition-colors" onclick="return confirm('<?= htmlspecialchars(t('admin_confirm_delete_challenge','Opravdu smazat?'), ENT_QUOTES) ?>')"><i class="fa-solid fa-trash"></i></a>
                                    </td>
                                </tr>
                                <?php endwhile; ?>
                            </tbody>
            ...
            <?php elseif ($section === 'leaderboard'): ?>
                <?php
                    ...
                    $medals     = ['🥇','🥈','🥉'];
                ?>
                <h1 class="text-4xl font-black text-gray-900 dark:text-white uppercase italic mb-2 tracking-tighter"><?= t('admin_leaderboard_title', '🏆 Žebříček') ?></h1>
                <p class="text-sm text-gray-500 mb-8"><?= t('admin_leaderboard_subtitle', 'Správa sezón – uzavři sezónu pro vyhlášení vítězů.') ?></p>
                ...
                <div class="glass rounded-3xl p-8 mb-8 border <?= $isClosed ? 'border-red-500/30' : 'border-emerald-500/30' ?> shadow-sm dark:shadow-none">
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        <h2 class="text-xl font-black text-gray-900 dark:text-white">
                            <?= t('admin_season_status', 'Stav') ?>: <span class="<?= $isClosed ? 'text-red-500' : 'text-emerald-500' ?>"><?= $isClosed ? t('admin_season_closed_status','UZAVŘENO') : t('admin_season_open_status','OTEVŘENO') ?></span>
                        </h2>
                        <?php if ($isClosed && $closedAt): ?>
                            <span class="text-xs text-gray-400 ml-auto"><?= t('admin_season_closed_at', 'Uzavřeno') ?>: <?= htmlspecialchars(date('d.m.Y H:i', strtotime($closedAt))) ?></span>
                        <?php endif; ?>
                    </div>

                    <?php if (!$isClosed): ?>
                    <!-- Formulář pro uzavření sezóny -->
                    <form method="POST" onsubmit="return confirm('<?= htmlspecialchars(t('admin_season_close_confirm','Uzavřít sezónu a vyhlásit Top 3 vítěze?'), ENT_QUOTES) ?>')">  
                        <div class="flex flex-col sm:flex-row gap-4 items-end">
                            <div class="flex-1 flex flex-col gap-1.5">
                                <label class="text-[10px] font-black uppercase tracking-widest text-gray-500"><?= t('admin_season_name_label', 'Název sezóny') ?></label>
                            ...
                            <button type="submit" name="close_leaderboard"
                                ...
                                <?= t('admin_season_close_btn', '🔒 Uzavřít sezónu') ?>
                            </button>
                        </div>
                        <p class="text-xs text-gray-400 mt-3"><?= t('admin_season_close_hint', 'Po uzavření se na žebříčku zobrazí podium s vítězi. Filtry a tabulka budou skryty.') ?></p>
                    </form>
                    <?php else: ?>
                    <!-- Zobrazení aktuálního snapshotu + možnost otevřít -->
                    <?php if (!empty($snapshot)): ?>
                    <div class="mb-6">
                        <p class="text-[10px] uppercase font-black tracking-widest text-gray-400 mb-4"><?= t('admin_season_winners_label', 'Uložení vítězové') ?> – „<?= htmlspecialchars($seasonLabel) ?>"</p>
                    ...
                    <?php endif; ?>
                    <form method="POST" onsubmit="return confirm('<?= htmlspecialchars(t('admin_season_open_confirm','Spustit novou sezónu? Podium zmizí a žebříček se vrátí do normálního režimu.'), ENT_QUOTES) ?>')">
                        <button type="submit" name="open_leaderboard"
                            ...
                            <?= t('admin_season_open_btn', '🔓 Spustit novou sezónu') ?>
                        </button>
                    </form>
                    <?php endif; ?>
                </div>
    ...
    function fillSelect(sel, data, valKey, lblKey, def) {
        ...
        sel.disabled = (data.length === 0 && def !== '<?= addslashes(t('register_select_country','Země')) ?>');
    }
    ...
    async function loadCountries() {
        ...
        fillSelect(cs, data, 'code', 'name', '<?= addslashes(t('register_select_country','Země')) ?>');
    ...
    async function loadRegions(cCode) {
        ...
        fillSelect(rs, data, 'id', 'name', '<?= addslashes(t('register_select_region','Kraj')) ?>');
    ...
    async function loadCities(rId, cCode) {
        ...
        fillSelect(ms, data, 'id', 'name', '<?= addslashes(t('register_select_city','Město')) ?>');
    }
