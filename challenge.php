...
if (isset($_GET['msg'])) {
    if ($_GET['msg'] === 'cancelled') $msg = "❌ Výzva byla zrušena a XP odečteny.";
    if ($_GET['msg'] === 'completed') $msg = "🎉 Gratulujeme! Výzva splněna, XP připsány.";
}
                ...
                <div class="max-w-4xl mx-auto">
                    <div class="glass rounded-[2.5rem] p-8 md:p-12 relative overflow-hidden shadow-xl">
                        ...
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                            <div>
                                <span class="text-emerald-500 font-bold text-[10px] uppercase tracking-[0.2em] mb-1 block">Detail Výzvy</span>
                                ...
                                <p class="text-gray-500 dark:text-gray-400 mt-2 flex items-center gap-2 text-sm font-bold uppercase tracking-wider">
                                    📍 <?= htmlspecialchars($ch['city_name'] ?: ($ch['region_name'] ?: 'Globální')) ?>
                                </p>
                            </div>
                            <div class="bg-yellow-500/10 border border-yellow-500/20 text-yellow-600 dark:text-yellow-400 px-6 py-3 rounded-2xl text-center">
                                <span class="block text-[10px] uppercase font-bold tracking-widest">Odměna</span>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                            <div class="bg-gray-100 dark:bg-white/5 p-4 rounded-2xl text-center">
                                ...
                                <div class="text-[10px] uppercase font-bold text-gray-500 tracking-widest">Cíl</div>
                                <div class="font-black text-gray-900 dark:text-white text-lg"><?= number_format($ch['goal_steps'], 0, '.', ' ') ?></div>
                            </div>
                            <div class="bg-gray-100 dark:bg-white/5 p-4 rounded-2xl text-center">
                                <div class="text-xl mb-1">⌛</div>
                                <div class="text-[10px] uppercase font-bold text-gray-500 tracking-widest">Limit</div>
                                <div class="font-black text-gray-900 dark:text-white text-lg"><?= $ch['time_limit_hours'] ?> hod</div>
                            </div>
                             <div class="bg-gray-100 dark:bg-white/5 p-4 rounded-2xl text-center border border-emerald-500/20">
                                <div class="text-xl mb-1">💰</div>
                                <div class="text-[10px] uppercase font-bold text-emerald-500 tracking-widest">Zisk</div>
                            ...
                            <div class="bg-gray-100 dark:bg-white/5 p-4 rounded-2xl text-center border border-red-500/20">
                                ...
                                <div class="text-[10px] uppercase font-bold text-red-500 tracking-widest">Penále</div>
                        ...
                        <?php if ($activeQuest && $activeQuest['challenge_id'] == $ch['id']): ?>
                            <div class="bg-gray-900/5 dark:bg-black/20 p-6 md:p-8 rounded-3xl border border-emerald-500/20 mb-8">
                                <h4 class="text-emerald-600 dark:text-emerald-400 font-black uppercase text-sm tracking-widest mb-4">📈 Tvůj aktuální progres</h4>
                                ...
                                <div class="flex justify-between text-xs font-bold uppercase tracking-wider mb-6 text-gray-500">
                                    <span><?= number_format($current_steps, 0, '.', ' ') ?> kroků</span>
                                    <span><?= round($percent, 1) ?> %</span>
                                    <span>Cíl: <?= number_format($ch['goal_steps'], 0, '.', ' ') ?></span>
                                </div>

                                <div class="grid grid-cols-3 gap-2 mb-6 text-center text-sm">
                                    <div class="bg-white dark:bg-white/5 p-3 rounded-xl">
                                        <span class="block text-gray-400 text-[10px] uppercase font-bold">Vzdálenost</span>
                                        <span class="font-bold dark:text-white"><?= number_format($syncData['distance_km'] ?? 0, 2) ?> km</span>
                                    </div>
                                    <div class="bg-white dark:bg-white/5 p-3 rounded-xl">
                                        <span class="block text-gray-400 text-[10px] uppercase font-bold">Kalorie</span>
                                        <span class="font-bold dark:text-white"><?= number_format($syncData['calories'] ?? 0, 0) ?> kcal</span>
                                    </div>
                                    <div class="bg-white dark:bg-white/5 p-3 rounded-xl">
                                        <span class="block text-gray-400 text-[10px] uppercase font-bold">Poslední Sync</span>
                                ...
                                <a href="src/fit/check_challenge.php?challenge_id=<?= $ch['id'] ?>" class="block w-full bg-emerald-600 hover:bg-emerald-500 text-white font-black py-4 rounded-xl text-center uppercase tracking-widest text-sm transition-all shadow-lg shadow-emerald-500/20">
                                    🔄 Aktualizovat data z Google Fit
                                </a>
                            </div>
                        <?php endif; ?>
        ...
        <?php else: ?>
            ...
            <?php if ($activeQuest): ?>
                <div class="mb-16">
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        <span class="text-emerald-600 dark:text-emerald-400 font-black uppercase tracking-widest text-sm">⚡ Právě plníš</span>
                        <div class="h-px bg-emerald-500/50 flex-grow"></div>
                    </div>
                    ...
                    <div class="relative overflow-hidden rounded-[2.5rem] quest-gradient border border-emerald-500/20 p-8 md:p-12 shadow-2xl">
                        ...
                        <div class="relative z-10">
                            <div class="flex flex-wrap items-center gap-3 mb-4">
                                <span class="bg-emerald-500 text-black text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter">Aktivní</span>
                                <span class="text-yellow-600 dark:text-yellow-400 font-bold text-xs uppercase tracking-widest">Odměna: <?= $activeQuest['xp_reward'] ?> XP</span>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-4 mt-8">
                                ...
                                <form method="post" onsubmit="return confirm('Opravdu to chceš vzdát? Ztratíš XP!')" class="inline-block">
            ...
            <?php else: ?>
                 <div class="glass rounded-[2rem] p-10 text-center border-dashed border-gray-300 dark:border-white/10 mb-16">
                    <p class="text-gray-500 italic">Žádná aktivní výzva. Vyber si nějakou z nabídky níže!</p>
                </div>
            <?php endif; ?>
            ...
            <div class="mb-12">
                <div class="flex flex-wrap items-center gap-4 mb-5">
                    ...
                    <h3 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-widest">Výzvy ve tvém okolí</h3>
                    <div class="h-px bg-gray-300 dark:bg-white/10 flex-grow"></div>
                </div>

                <div class="flex flex-wrap items-center gap-2 mb-6">
                    <span class="text-xs font-black uppercase tracking-wider text-gray-500">Okruh:</span>
                    ...
                    <?php endforeach; ?>
                    <?php if ($userLat === null): ?>
                        <span class="text-xs text-orange-500 font-bold ml-1">⚠️ Nastav město v profilu pro GPS filtr</span>
                    <?php endif; ?>
                </div>

                <?php if (empty($localChallenges)): ?>
                    <p class="text-gray-500 dark:text-gray-500 italic text-sm">Žádné výzvy v okruhu <?= $localRadius ?> km od tvého města.</p>
                <?php else: ?>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <?php foreach ($localChallenges as $ch_item): ?>
                            <div class="glass p-6 rounded-3xl relative group hover:border-yellow-500/30 transition-all flex flex-col h-full">
                                ...
                                <small class="text-gray-400 font-bold uppercase text-[10px] tracking-wider mb-6 block">
                                    📍 <?= htmlspecialchars($ch_item['city_name'] ?: $ch_item['region_name'] ?: $ch_item['country_code'] ?: 'Globální') ?>
                                </small>
                                <div class="mt-auto">
            ...
            <div class="mb-12">
                <div class="flex flex-wrap items-center gap-4 mb-5">
                    <span class="text-2xl">🌍</span>
                    <h3 class="text-xl font-black text-gray-900 dark:text-white uppercase tracking-widest">Všechny výzvy</h3>
                    <div class="h-px bg-gray-300 dark:bg-white/10 flex-grow"></div>
                </div>
                ...
                <form method="get" class="glass rounded-2xl p-4 mb-6 flex flex-wrap gap-3 items-end">
                    ...
                    <div class="flex flex-col gap-1">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-500">Řadit</label>
                        <select name="g_sort" onchange="this.form.submit()" class="bg-white dark:bg-white/5 border border-gray-300 dark:border-white/10 rounded-xl px-3 py-2 text-sm font-bold text-gray-800 dark:text-white focus:outline-none focus:border-blue-500">
                            <option value="distance" <?= $gSort === 'distance' ? 'selected' : '' ?>>📍 Vzdálenost</option>
                            <option value="xp"       <?= $gSort === 'xp'       ? 'selected' : '' ?>>⚡ XP odměna</option>
                            <option value="steps"    <?= $gSort === 'steps'    ? 'selected' : '' ?>>👟 Náročnost</option>
                        </select>
                    </div>

                    <?php if (!empty($filterCountries)): ?>
                    <div class="flex flex-col gap-1">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-500">Stát</label>
                    ...
                    <div class="flex flex-col gap-1">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-500">Kraj</label>
                    ...
                    <?php if ($userLat !== null): ?>
                    <div class="flex flex-col gap-1">
                        <label class="text-[10px] font-black uppercase tracking-widest text-gray-500">Max. km ode mě</label>
                        <select name="g_km" onchange="this.form.submit()" class="bg-white dark:bg-white/5 border border-gray-300 dark:border-white/10 rounded-xl px-3 py-2 text-sm font-bold text-gray-800 dark:text-white focus:outline-none focus:border-blue-500">
                            <option value="0"   <?= $gMaxKm === 0   ? 'selected' : '' ?>>Bez limitu</option>
                    ...
                    <?php if ($gCountry || $gRegion || $gMaxKm): ?>
                        <a href="?local_radius=<?= $localRadius ?>" class="text-xs font-bold text-red-500 hover:text-red-400 transition-colors self-end pb-2">✕ Zrušit filtry</a>
                    <?php endif; ?>
                </form>

                <?php if (empty($globalChallenges)): ?>
                    <p class="text-gray-500 dark:text-gray-500 italic text-sm">Žádné výzvy neodpovídají filtru.</p>
                <?php else: ?>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <?php foreach ($globalChallenges as $ch_item): ?>
                            <div class="glass p-6 rounded-3xl relative group hover:border-blue-500/30 transition-all flex flex-col h-full">
                                ...
                                <small class="text-gray-400 font-bold uppercase text-[10px] tracking-wider mb-4 block">
                                    📍 <?= htmlspecialchars($ch_item['city_name'] ?: $ch_item['region_name'] ?: $ch_item['country_code'] ?: 'Globální') ?>
                                </small>
                                <div class="mt-auto">
